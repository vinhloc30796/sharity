...
**Маленькие PR'ы:** Договорились с Vinh Loc делать частые маленькие PR вместо больших.

- **Одна фича = один PR** — не накапливать много изменений
- **Merge часто** — чтобы избежать конфликтов
- **`pnpm format` перед коммитом** — Husky делает это автоматически

**Перед началом работы:**
1. `git pull origin main` — забрать последние изменения
2. Создать ветку для одной конкретной фичи
3. Сделать PR как только фича готова

**Почему это важно:**
- PR #4 был слишком большой (11 коммитов, 21 файл)
- Возникли конфликты в `app/page.tsx` и `profile-form.tsx`
- Лок потратил время на разрешение конфликтов
