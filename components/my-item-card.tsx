...
export function MyItemCard({ item, isOwner = true }: { item: Doc<"items">, isOwner?: boolean }) {
  ...
  const claims = useQuery(api.items.getClaims, isOwner ? { itemId: item._id } : "skip");
          ...
          <div className="flex justify-end gap-2 w-full">
            ...
            <div className="flex justify-end gap-2 w-full">
                {/* Placeholder for Return functionality */}
                 <Button variant="outline" size="sm" disabled>Return (Coming Soon)</Button>
            </div>
          )
        }
    >
            ...
            <div className="mt-4 p-3 bg-yellow-50 rounded-md border border-yellow-200">
                ...
                <div className="space-y-2">
                    {pendingClaims.map(claim => (
                        <div key={claim._id} className="flex flex-col gap-2 text-sm bg-white p-2 rounded border">
                            <span className="font-medium">User:</span>
                            <span className="text-xs text-gray-500 break-all">{claim.claimerId}</span>
