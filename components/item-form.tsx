...
interface ItemFormProps {
  ...
  onSubmit: (values: { name: string; description: string; imageStorageIds?: Id<"_storage">[] }) => Promise<void>;
  submitLabel?: string;
}
...
export function ItemForm({ initialValues, onSubmit, submitLabel = "Submit" }: ItemFormProps) {
  ...
  const [existingImages, setExistingImages] = useState<{ id: Id<"_storage">; url: string }[]>(() => {
      ...
      if (initialValues?.imageStorageIds && initialValues?.imageUrls) {
          ...
          if (initialValues.imageStorageIds.length === initialValues.imageUrls.length) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setPendingUploads(files.length); // Visual indicator if needed, though we use button text
    try {
      // 1. Upload new files
      ...
      await onSubmit({ 
          name, 
          description, 
          imageStorageIds: finalStorageIds.length > 0 ? finalStorageIds : undefined 
      });
            ...
            <div className="grid grid-cols-5 gap-2 mb-2">
                {existingImages.map((img) => (
                    <div key={img.id} className="relative group aspect-square">
                        ...
                        <button
                            type="button"
                            onClick={() => setExistingImages(prev => prev.filter(p => p.id !== img.id))}
